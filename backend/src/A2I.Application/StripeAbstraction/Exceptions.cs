...
{
    public static Exception Wrap(StripeException ex, string? overrideMessage = null)
    {
        ...
        if ((int?)ex.HttpStatusCode == 404 ||
            string.Equals(ex.StripeError?.Code, "resource_missing", StringComparison.OrdinalIgnoreCase))
