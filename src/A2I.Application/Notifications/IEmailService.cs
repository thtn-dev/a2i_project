...
public interface IEmailService
{
    ...
    Task SendPaymentFailedEmailAsync(Guid customerId, Guid invoiceId, long attemptCount, DateTime? nextRetry,
        CancellationToken ct = default);

    Task SendCancellationEmailAsync(Guid customerId, string planName, DateTime endDate, CancellationToken ct = default);

    Task SendTrialEndingEmailAsync(Guid customerId, string planName, DateTime trialEndDate,
        CancellationToken ct = default);

    Task SendPaymentActionRequiredEmailAsync(Guid customerId, Guid invoiceId, string actionUrl,
        CancellationToken ct = default);
}